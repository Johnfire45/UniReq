...
public class RequestDeduplicator {
    ...
    /**
     * RequestResponseEntry - Stores a unique request/response pair with metadata
     * 
     * This inner class represents a single HTTP transaction that was deemed unique.
     * It includes the original request, response (when available), timestamp, and
     * truncated previews for efficient UI display.
     */
    public static class RequestResponseEntry {
        ...
        /**
         * Creates a safe preview of content by truncating and removing sensitive headers
         */
        private String createSafePreview(String content) {
            if (content == null) return "";
            
            // Remove sensitive headers (Authorization, Cookie, etc.)
            String sanitized = content.replaceAll("(?i)(Authorization|Cookie|X-API-Key|Bearer):[^\r\n]*", "$1: [REDACTED]");
        ...
        public String getPath() { return request.path(); }
